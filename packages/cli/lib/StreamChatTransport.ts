...
export type StreamFunc<UI_MESSAGE extends UIMessage> = (options: {
    messages: UI_MESSAGE[];
    ...
    implements ChatTransport<UI_MESSAGE>
{
    ...
    ) => Promise<ReadableStream<UIMessageStreamPart>> = async (options) => {
        return this.func({ messages: options.messages });
    };
    reconnectToStream: (
        options: { chatId: string } & ChatRequestOptions,
    ) => Promise<ReadableStream<UIMessageStreamPart> | null> = async () => {
